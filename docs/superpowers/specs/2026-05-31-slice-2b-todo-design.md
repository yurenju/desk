...
`GET /todo/items` 支援用 custom field 過濾,但**主 OpenAPI（`https://api.wspc.ai/openapi.json`）並沒有文件化這個參數**(2026-06-01 重新確認仍未文件化)。它只出現在 WSPC 的 MCP tool schema 與 `https://wspc.ai/llms.txt`。本片用 `scripts/verify-wspc.mjs`（零依賴 Node 工具，走完整 device flow）對線上 API 實打驗證,並作為回歸檢查保留。

| 行為 | 實測結果(2026-06-01) |
|---|---|
| `?cf.scheduled_dates=<date>`（點號語法） | ✅ 對 `string_array` 做真正的 **array-contains** 過濾 |
| `?cf[scheduled_dates]=<date>`（中括號語法） | ❌ HTTP 400(早期會靜默忽略,2026-06 起明確拒絕) |
| `?cf.<未宣告欄位>=x` | ✅ HTTP 422 `UNKNOWN_CUSTOM_FIELD`(早期會靜默回整包,2026-06 起明確拒絕) |
...
2. 過去「打錯 cf key 靜默回整包」的危險已被 WSPC 修掉;但 `cf` 仍未進主 OpenAPI,且 `cf.scheduled_dates` 是我們唯一仰賴的過濾條件——維持「常數 + 鎖定組裝結果的測試」作為防呆與單一真實來源(`worker/wspc.ts` `CF_SCHEDULED_DATES`)。
...
過濾欄位名寫成常數（`const CF_SCHEDULED_DATES = "cf.scheduled_dates"`,鎖死於 `worker/wspc.ts`），並加單元測試鎖死字串與 query 組裝結果。

當初的動機是「打錯 cf key 會靜默回整包」這個 server 端漏洞;2026-06 起 WSPC 已改成回 422 `UNKNOWN_CUSTOM_FIELD`,但常數+測試的防呆仍然保留——`cf` 未進主 OpenAPI、單一過濾條件、且讓 typo 在開發期就被擋下,是低成本的紀律。
...
- **cf key 常數鎖定**：assert query 組裝字串為 `cf.scheduled_dates=...`,維持單一真實來源並擋下重構期的 typo。
